...
;A good example is the English-language sentence
;"[The quick brown fox jumps over the lazy dog](http://en.wikipedia.org/wiki/The_quick_brown_fox_jumps_over_the_lazy_dog)";
...
;## Input Description
;
;On standard console input, you will be given a single integer on the first line of input.
;This integer represents the number of lines you will then receive,
;each being a string of alpha-numeric characters ('a'-'z', 'A'-'Z', '0'-'9')
;as well as spaces and [period](http://en.wikipedia.org/wiki/Period_(punctuation\)).
;
;## Output Description
;
...
(define pangram?
  (lambda (sentence)
    (if
      (= (string-length (string-delete (->char-set (string-downcase sentence)) alphabet)) 0)
        (display "True\n")
        (display "False\n"))))
...
(pangram? "Saxophones quickly blew over my jazzy hair")

;misc input
(pangram? "Nymphs blitz quick vex dwarf jog. ")
(pangram? "I am a very happy man oh yes")
